...
def box(name,loc,scale,mat,bevel=.025):
 ...
 return o
box('Bench pad',(0,.15,.432),(.3,1.22,.085),pad)
...
bpy.ops.object.empty_add();bar=bpy.context.object;bar.name='Barbell'
def cylinder(name,x,radius,depth,mat):
 bpy.ops.mesh.primitive_cylinder_add(vertices=64,radius=radius,depth=depth,location=(x,0,0),rotation=(0,math.pi/2,0));o=bpy.context.object;o.name=name;o.parent=bar;o.data.materials.append(mat)
 b=o.modifiers.new('Rounded metal edges','BEVEL');b.width=min(.008,depth*.12);b.segments=3;bpy.ops.object.modifier_apply(modifier=b.name)
 for p in o.data.polygons:p.use_smooth=True
 return o
cylinder('Bar shaft',0,.014,1.80,steel)
for side in (-1,1):
 cylinder('Bar sleeve',side*.74,.025,.31,steel)
 cylinder('Weight plate',side*.68,.20,.075,rubber)
 cylinder('Weight plate rim',side*.72,.17,.012,frame)
 cylinder('Bar collar',side*.60,.039,.031,steel)
 cylinder('Outer collar',side*.755,.036,.025,steel)
...
for frame_index in range(121):
 ...
 bar.location=(0,.245-.065*down,1.095-.355*down)
 bar.keyframe_insert(data_path='location')
 errors=[]
 for side,sign in [('l',1),('r',-1)]:
  rotation,contact=hands[side]
  grip=bar.location+Vector((sign*.32,0,0));wrist=grip-contact
 ...
 metrics.append({'frame':frame_index,'bar':list(bar.location),'palm_anchor_error_m':max(errors)})
# Linear samples of smooth quintic movement; identical end points close the loop.
for obj in (rig,bar):
 if obj.animation_data and obj.animation_data.action:
  obj.animation_data.action.name='BenchPressLoop_'+obj.name
...
(ROOT/'motion_metrics.json').write_text(json.dumps(metrics,indent=2))
...
scene.frame_set(1)
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'bench.blend'))
for f,name in [(1,'top'),(56,'bottom')]:
 scene.frame_set(f);scene.render.filepath=str(ROOT/f'bench_{name}.png');bpy.ops.render.render(write_still=True)
