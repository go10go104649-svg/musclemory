...
if __name__ == '__main__':
    parser=argparse.ArgumentParser();parser.add_argument('source', type=Path);parser.add_argument('--prune', action='store_true');args=parser.parse_args()
    DEST.mkdir(parents=True, exist_ok=True);CHUNKS.mkdir(parents=True, exist_ok=True)
    raw=0
    for source in sorted(args.source.glob('*.glb')): raw+=pack(source, source.stem)
    ...
    print(json.dumps({'unpacked_source_bytes':raw,'packed_bundle_bytes':packed,'unique_chunks':len(referenced),'recipes':len(list(DEST.glob('*.form.json'))),'orphan_bytes':orphan_bytes,'pruned':args.prune},indent=2))
