...
アプリ名は **SETKEEP TRAINER**、両OSの識別子は **com.setkeep.trainer**。一般版 **com.setkeep.app** と共存します。SETKEEP本体の正式カラーは `#C7F36B`、TRAINERは現行mainのアプリ固有アクセント `#38C6FF` を使用します。スマートフォンは下部4タブ、幅720以上はサイドナビゲーション。日本語・英語のUI文言を用意しています。ブランド、料金、通知、休憩タイマー、3D進捗の現行仕様は [current_spec.md](current_spec.md) を正本とします。
...
- `lib/trainer/trainer_sharing_page.dart`: 一般版での招待確認・承認・解除・共有。代理記録の手動受信操作は廃止。
- `test/trainer_sharing_test.dart` / `test/trainer_workout_sync_test.dart`: 明示承認、権限初期値、自動同期、要求ID。
...
- `lib/main.dart`: 一般版の「Trainerと連携」の遷移先と、代理記録の自動同期。Supabase `workouts.id` をローカル履歴に保持し、同時刻の本人記録とは別に扱います。
...
一般版は起動後、ログイン完了後、バックグラウンドからの復帰時、Trainer連携承認後に本人の `user_id` と `record_source='trainer'` に一致する `workouts` を100件ずつ最後まで取得します。全ページ成功後に限りローカル履歴を更新します。`workouts.id` と所有ユーザーIDで重複を防ぎ、`canceled_at` がある記録は対応するID付き代理記録だけ除外、復元時は再表示します。ホーム・履歴・月間カレンダー・部位などは同じ履歴一覧を参照します。通信失敗や未ログイン時は既存履歴を変更しません。本人記録は代理記録同期の削除対象外です。旧手動受信済みデータには出所を示すIDがありません。日時・ジム・所要時間・全セットが一意に一致する場合は二重追加を抑えますが、本人記録の誤削除を防ぐためIDの付与や取消による削除は行いません。旧記録の内容が後から変わる場合や取消時には旧記録が残る可能性があります。Premium向けクラウドバックアップとオフライン送信キュー、編集の双方向同期は別機能です。

## 8. 未実装・制約

- 予約、チャット、AI、課金、決済、売上、SETKEEP BUSINESSの実課金。現行仕様で定義するTRAINER右上ベル（メニューコメント等の新着通知と未読数バッジ）も、この初期実装記録の時点では未実装です。
...
1. 2つの実アカウントを使い、Android / iPhoneで招待→承認→共有→代理記録→一般版への自動反映→取消・復元→解除を通して確認。
