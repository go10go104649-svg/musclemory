...
当該tenantの `status=active AND is_trainer` を数えます。Adminのみは0、兼任は1、招待待ちは0、同じユーザーでも別tenantでは別に算定。SETKEEP BUSINESS / SETKEEP TRAINERの正式料金は月額3,980円で5IDまで含み、6ID目以降は1IDあたり月額500円追加です。サーバーの見積は `3980 + max(count - 5, 0) * 500` 円/月。旧仕様の「4,980円／店舗・トレーナー無制限」は使用しません。実課金はまだ行いません。
...
TRAINERのaccent / Primaryは現行mainの `#38C6FF`。薄い背景と選択色もFamilyPaletteに集約。小さい文字のボタンは暗い前景色にして可読性を保ちます。旧 `#00D084` と旧資料の `#79D5F6` は正式色として使用しません。一般版は正式カラー `#C7F36B` と既存ThemeDataを維持し、テーマ等価性テストで検証します。
...
Galaxy / iPhone実機でのGoogleログインと本人同一性、2つの実アカウントでの招待から解除までの通し確認は残っています。料金仕様は確定していますが、Stripe Checkout・検証済みWebhook・実請求・支払状態に応じた提供制御、詳細な監査管理画面、体重データの共有、端末履歴との取消双方向同期、繰返し予定の自動展開は未実装です。BUSINESSの実課金には着手していません。
