...
全46種目の再制作は未完了。公開再開は12種目（下表の確認完了分。配布・pushなし）。元の2種目と合わせ通常公開14種目、未完了34種目はauthoredのまま非公開。15種目の修正アセットをパック済みだが、未確認の3種目は公開しない。インクラインバーベルは押し上げ終点の問題で一度完了判定を撤回した後、新軌道で再生成し両OS・通常導線を再確認済み。旧レシピは `build/exercise_forms/rebuild_20260922/previous-recipes` に保持。元の2種目は変更しない。
