...
Rebuild scripts: tool/bench_press/{make_base,pose_bench,export_bench}.py, in that order through Blender --background --factory-startup --python-exit-code 1 --python SCRIPT. Set MPFB_REPO to the checked-out MPFB2 repository and MUSCLEMORY_ART_WORK to an existing scratch folder. Export uses the project's assets directory. An installed MPFB extension is unnecessary; make_base registers the source in an isolated extension repository. Use a separate BLENDER_USER_CONFIG/BLENDER_USER_EXTENSIONS to preserve personal Blender settings.
